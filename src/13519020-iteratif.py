...
def topSort():
    ...
    # Dicopy dulu biar ga ngeubah yang global bray
    olahMatkul = {key: value[:] for key, value in graphMatkul.items()}
    
    # Looping selama olahMatkul belum kosong, WARNING pastikan graph adalah DAC
    while (len(olahMatkul) > 0):
        found = False
        
        # Cari matkul yang sudah tidak punya prerequisite
        for namaMatkul in olahMatkul:
            if len(olahMatkul[namaMatkul]) == 0:    
        ...
        # Iterasi, ulang lagi dari awal untuk graph yang telah dikurangi (Decrease and Conquare)
        else:
